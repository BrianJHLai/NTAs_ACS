...
povertyA <- get_acs(geography = "tract",
                   year = 2010, table = c("B17020A"),
                   state = "NY", county = c("Queens"),
                   survey = "acs5",
                   geometry = TRUE, output="wide")

povertyB <- get_acs(geography = "tract",
                    year = 2010, table = c("B17020B"),
                    state = "NY", county = c("Queens"),
                    survey = "acs5",
                    geometry = TRUE, output="wide")

povertyC <- get_acs(geography = "tract",
                    year = 2010, table = c("B17020C"),
                    state = "NY", county = c("Queens"),
                    survey = "acs5",
                    geometry = TRUE, output="wide")

povertyD <- get_acs(geography = "tract",
                    year = 2010, table = c("B17020D"),
                    state = "NY", county = c("Queens"),
                    survey = "acs5",
                    geometry = TRUE, output="wide")

povertyE <- get_acs(geography = "tract",
                    year = 2010, table = c("B17020E"),
                    state = "NY", county = c("Queens"),
                    survey = "acs5",
                    geometry = TRUE, output="wide")

povertyF <- get_acs(geography = "tract",
                    year = 2010, table = c("B17020F"),
                    state = "NY", county = c("Queens"),
                    survey = "acs5",
                    geometry = TRUE, output="wide")

povertyG <- get_acs(geography = "tract",
...
ntas_tracts_poverty_summary <- ntas_tracts_poverty %>%
  ...
  #summarize(total = mean(B17020A_001E, B17020B_001E, B17020C_001E, B17020D_001E, B17020E_001E, B17020F_001E, B17020G_001E, B17020H_001E, B17020I_001E, na.rm = TRUE, trim = 0),
            #poverty = mean(B17020A_002E, B17020B_002E, B17020C_002E, B17020D_002E, B17020E_002E, B17020F_002E, B17020G_002E, B17020H_002E, B17020I_002E, na.rm = TRUE, trim = 0)/total)
  #summarize(total = sum(B17020_001E, na.rm = TRUE),                             
...
#write to csv file                                                              #$$$$$$$$$$$
write.csv(ntas_tracts_hh_over_60_summary, "hh_over_60_2010.csv")
...
#write to csv file                                                              #$$$$$$$$$$$
write.csv(ntas_tracts_hh_living_alone_summary, "hh_living_alone_2010.csv")
...
hh_languages_2010 <- get_acs(geography = "tract",
...
#ntas_tracts <- ntas_tracts %>% mutate(GEOID=as.character(GEOID))
ntas_hh_languages_2010 <- ntas_tracts %>% left_join(hh_languages_2010, by="GEOID")

#group data on hh languages by library service area
hh_languages_2010_summary <- ntas_hh_languages_2010 %>%
  group_by(`nta_name`) %>%
